...
const initialRows = [
  {
    ...
    "origen": "Busan",
    "productos": "Pastillas freno Accent",
    "leadTime": "32 días",
    "ventana": "Junio 2026",
    "costo": "US$ 5,800",
    "escenario": "Rentable",
...
const filterFields = ["origen", "escenario", "estado"];
const actions = [{"id": "view", "label": "Ver detalle"}, {"id": "edit", "label": "Editar"}, {"id": "approve", "label": "Aprobar escenario"}];
const formFields = ["proveedor", "origen", "productos", "leadTime", "ventana", "costo", "escenario", "estado"];
...
export default function ManagerLogisticsPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      const matchesText = !term || normalize((row.proveedor ?? "") + " " + (row.origen ?? "") + " " + (row.productos ?? "") + " " + (row.leadTime ?? "") + " " + (row.ventana ?? "") + " " + (row.costo ?? "") + " " + (row.escenario ?? "") + " " + (row.estado ?? "")).includes(term);
  ...
  function openCreate() {
    ...
    setModal({ type: "form", title: "Nuevo escenario", mode: "create" });
  }
  ...
  function runAction(actionId, row) {
    ...
    if (actionId === "download") { downloadCsv("manager-logistics.csv", [row]); showToast("Archivo CSV descargado"); return; }
    showToast("Acción simulada correctamente");
  }

  return (
    <main className="manager-logistics-page">
      <section className="manager-logistics-hero">
        <div>
          <span className="manager-logistics-eyebrow">Gerente de importaciones</span>
          <h2>Logística de importación</h2>
          <p>Lead time, proveedores y escenarios. Este módulo tiene su propio código, estilos, estados, filtros, tabla y ventana dentro de <strong>src/features/manager/logistics</strong>.</p>
        </div>
        <div className="manager-logistics-hero-actions">
          <button onClick={openCreate}>Nuevo escenario</button>
          <button onClick={() => downloadCsv("manager-logistics.csv", filteredRows)}>Exportar CSV</button>
        </div>
      </section>

      <section className="manager-logistics-toolbar">
        <label className="manager-logistics-search">Buscar
      ...
      <section className="manager-logistics-summary">
      ...
      <section className="manager-logistics-table-card">
        <div className="manager-logistics-table-head">
          <div>
            <span>Vista operativa</span>
            <h3>Logística de importación</h3>
          </div>
          <small>{filteredRows.length} resultados</small>
        </div>
        <div className="manager-logistics-table-wrap">
          <table>
            <thead>
              <tr>
                  ...
                  {columns.map((column) => (
                    <td key={column.key}>
                      {["estado", "prioridad", "riesgo", "rotacion", "severidad", "probabilidad", "concentracion", "escenario"].includes(column.key)
                        ? <span className={`manager-logistics-badge ${badgeClass(row[column.key])}`}>{row[column.key]}</span>
                        : row[column.key]}
                    </td>
                  ))}
                  <td>
                    <div className="manager-logistics-row-actions">
                      {actions.map((action) => <button key={action.id} onClick={() => runAction(action.id, row)}>{action.label}</button>)}
                    </div>
                  </td>
                </tr>
              ))}
              {filteredRows.length === 0 && <tr><td colSpan={columns.length + 1} className="manager-logistics-empty">No hay resultados para los filtros seleccionados.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>

      {toast && <div className="manager-logistics-toast">{toast}</div>}

      {modal?.type === "detail" && (
        <div className="manager-logistics-modal-backdrop" onClick={() => setModal(null)}>
          <section className="manager-logistics-modal" onClick={(event) => event.stopPropagation()}>
            <button className="manager-logistics-close" onClick={() => setModal(null)}>×</button>
            <span className="manager-logistics-eyebrow">Detalle</span>
            <h3>Información del registro</h3>
            <div className="manager-logistics-detail-grid">
              {columns.map((column) => (
            ...
            <div className="manager-logistics-modal-actions"><button onClick={() => setModal(null)}>Cerrar</button></div>
          </section>
        </div>
      )}
      ...
      {modal?.type === "form" && (
        <div className="manager-logistics-modal-backdrop" onClick={() => setModal(null)}>
          <form className="manager-logistics-modal manager-logistics-form" onSubmit={saveForm} onClick={(event) => event.stopPropagation()}>
            <button type="button" className="manager-logistics-close" onClick={() => setModal(null)}>×</button>
            <span className="manager-logistics-eyebrow">Formulario</span>
            <h3>{modal.title}</h3>
            <div className="manager-logistics-form-grid">
            ...
            <div className="manager-logistics-modal-actions">
