...
const actions = [{"id": "view", "label": "Ver detalle"}, {"id": "edit", "label": "Editar"}, {"id": "approve", "label": "Aprobar"}, {"id": "reject", "label": "Rechazar"}];
...
export default function ManagerImportRecommendationsPage() {
  ...
  function runAction(actionId, row) {
    ...
    if (actionId === "download") { downloadCsv("manager-import-recommendations.csv", [row]); showToast("Archivo CSV descargado"); return; }
    showToast("Acción simulada correctamente");
  }

  return (
    <main className="manager-import-recommendations-page">
      <section className="manager-import-recommendations-hero">
        <div>
          <span className="manager-import-recommendations-eyebrow">Gerente de importaciones</span>
          <h2>Aprobación de importaciones</h2>
          <p>Aprobar, rechazar y priorizar productos. Este módulo tiene su propio código, estilos, estados, filtros, tabla y ventana dentro de <strong>src/features/manager/import-recommendations</strong>.</p>
        </div>
        <div className="manager-import-recommendations-hero-actions">
          <button onClick={openCreate}>Crear propuesta</button>
          <button onClick={() => downloadCsv("manager-import-recommendations.csv", filteredRows)}>Exportar CSV</button>
        </div>
      </section>

      <section className="manager-import-recommendations-toolbar">
        <label className="manager-import-recommendations-search">Buscar
      ...
      <section className="manager-import-recommendations-summary">
      ...
      <section className="manager-import-recommendations-table-card">
        <div className="manager-import-recommendations-table-head">
          <div>
            <span>Vista operativa</span>
            <h3>Aprobación de importaciones</h3>
          </div>
          <small>{filteredRows.length} resultados</small>
        </div>
        <div className="manager-import-recommendations-table-wrap">
          <table>
            <thead>
              <tr>
                  ...
                  {columns.map((column) => (
                    <td key={column.key}>
                      {["estado", "prioridad", "riesgo", "rotacion", "severidad", "probabilidad", "concentracion", "escenario"].includes(column.key)
                        ? <span className={`manager-import-recommendations-badge ${badgeClass(row[column.key])}`}>{row[column.key]}</span>
                        : row[column.key]}
                    </td>
                  ))}
                  <td>
                    <div className="manager-import-recommendations-row-actions">
                      {actions.map((action) => <button key={action.id} onClick={() => runAction(action.id, row)}>{action.label}</button>)}
                    </div>
                  </td>
                </tr>
              ))}
              {filteredRows.length === 0 && <tr><td colSpan={columns.length + 1} className="manager-import-recommendations-empty">No hay resultados para los filtros seleccionados.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>

      {toast && <div className="manager-import-recommendations-toast">{toast}</div>}

      {modal?.type === "detail" && (
        <div className="manager-import-recommendations-modal-backdrop" onClick={() => setModal(null)}>
          <section className="manager-import-recommendations-modal" onClick={(event) => event.stopPropagation()}>
            <button className="manager-import-recommendations-close" onClick={() => setModal(null)}>×</button>
            <span className="manager-import-recommendations-eyebrow">Detalle</span>
            <h3>Información del registro</h3>
            <div className="manager-import-recommendations-detail-grid">
              {columns.map((column) => (
            ...
            <div className="manager-import-recommendations-modal-actions"><button onClick={() => setModal(null)}>Cerrar</button></div>
          </section>
        </div>
      )}
      ...
      {modal?.type === "form" && (
        <div className="manager-import-recommendations-modal-backdrop" onClick={() => setModal(null)}>
          <form className="manager-import-recommendations-modal manager-import-recommendations-form" onSubmit={saveForm} onClick={(event) => event.stopPropagation()}>
            <button type="button" className="manager-import-recommendations-close" onClick={() => setModal(null)}>×</button>
            <span className="manager-import-recommendations-eyebrow">Formulario</span>
            <h3>{modal.title}</h3>
            <div className="manager-import-recommendations-form-grid">
            ...
            <div className="manager-import-recommendations-modal-actions">
