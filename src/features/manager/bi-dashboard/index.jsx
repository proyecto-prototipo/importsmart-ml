...
const formFields = ["producto", "region", "cantidad", "prioridad", "riesgo", "margen", "estado"];
...
export default function ManagerBiDashboardPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      const matchesText = !term || normalize((row.producto ?? "") + " " + (row.region ?? "") + " " + (row.cantidad ?? "") + " " + (row.prioridad ?? "") + " " + (row.riesgo ?? "") + " " + (row.margen ?? "") + " " + (row.estado ?? "")).includes(term);
  ...
  function runAction(actionId, row) {
    ...
    if (actionId === "download") { downloadCsv("manager-bi-dashboard.csv", [row]); showToast("Archivo CSV descargado"); return; }
    showToast("Acción simulada correctamente");
  }

  return (
    <main className="manager-bi-dashboard-page">
      <section className="manager-bi-dashboard-hero">
        <div>
          <span className="manager-bi-dashboard-eyebrow">Gerente de importaciones</span>
          <h2>Dashboard BI ejecutivo</h2>
          <p>Rentabilidad, riesgo y oportunidad. Este módulo tiene su propio código, estilos, estados, filtros, tabla y ventana dentro de <strong>src/features/manager/bi-dashboard</strong>.</p>
        </div>
        <div className="manager-bi-dashboard-hero-actions">

          <button onClick={() => downloadCsv("manager-bi-dashboard.csv", filteredRows)}>Exportar CSV</button>
        </div>
      </section>

      <section className="manager-bi-dashboard-toolbar">
        <label className="manager-bi-dashboard-search">Buscar
      ...
      <section className="manager-bi-dashboard-summary">
      ...
      <section className="manager-bi-dashboard-table-card">
        <div className="manager-bi-dashboard-table-head">
          <div>
            <span>Vista operativa</span>
            <h3>Dashboard BI ejecutivo</h3>
          </div>
          <small>{filteredRows.length} resultados</small>
        </div>
        <div className="manager-bi-dashboard-table-wrap">
          <table>
            <thead>
              <tr>
                  ...
                  {columns.map((column) => (
                    <td key={column.key}>
                      {["estado", "prioridad", "riesgo", "rotacion", "severidad", "probabilidad", "concentracion", "escenario"].includes(column.key)
                        ? <span className={`manager-bi-dashboard-badge ${badgeClass(row[column.key])}`}>{row[column.key]}</span>
                        : row[column.key]}
                    </td>
                  ))}
                  <td>
                    <div className="manager-bi-dashboard-row-actions">
                      {actions.map((action) => <button key={action.id} onClick={() => runAction(action.id, row)}>{action.label}</button>)}
                    </div>
                  </td>
                </tr>
              ))}
              {filteredRows.length === 0 && <tr><td colSpan={columns.length + 1} className="manager-bi-dashboard-empty">No hay resultados para los filtros seleccionados.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>

      {toast && <div className="manager-bi-dashboard-toast">{toast}</div>}

      {modal?.type === "detail" && (
        <div className="manager-bi-dashboard-modal-backdrop" onClick={() => setModal(null)}>
          <section className="manager-bi-dashboard-modal" onClick={(event) => event.stopPropagation()}>
            <button className="manager-bi-dashboard-close" onClick={() => setModal(null)}>×</button>
            <span className="manager-bi-dashboard-eyebrow">Detalle</span>
            <h3>Información del registro</h3>
            <div className="manager-bi-dashboard-detail-grid">
              {columns.map((column) => (
            ...
            <div className="manager-bi-dashboard-modal-actions"><button onClick={() => setModal(null)}>Cerrar</button></div>
          </section>
        </div>
      )}
      ...
      {modal?.type === "form" && (
        <div className="manager-bi-dashboard-modal-backdrop" onClick={() => setModal(null)}>
          <form className="manager-bi-dashboard-modal manager-bi-dashboard-form" onSubmit={saveForm} onClick={(event) => event.stopPropagation()}>
            <button type="button" className="manager-bi-dashboard-close" onClick={() => setModal(null)}>×</button>
            <span className="manager-bi-dashboard-eyebrow">Formulario</span>
            <h3>{modal.title}</h3>
            <div className="manager-bi-dashboard-form-grid">
            ...
            <div className="manager-bi-dashboard-modal-actions">
