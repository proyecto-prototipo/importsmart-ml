...
export default function AnalystDemandForecastPage() {
  ...
  function runAction(actionId, row) {
    ...
    if (actionId === "download") { downloadCsv("analyst-demand-forecast.csv", [row]); showToast("Archivo CSV descargado"); return; }
    showToast("Acción simulada correctamente");
  }

  return (
    <main className="analyst-demand-forecast-page">
      <section className="analyst-demand-forecast-hero">
        <div>
          <span className="analyst-demand-forecast-eyebrow">Analista comercial</span>
          <h2>Forecasting de demanda</h2>
          <p>Pronósticos por producto, región y período. Este módulo tiene su propio código, estilos, estados, filtros, tabla y ventana dentro de <strong>src/features/analyst/demand-forecast</strong>.</p>
        </div>
        <div className="analyst-demand-forecast-hero-actions">

          <button onClick={() => downloadCsv("analyst-demand-forecast.csv", filteredRows)}>Exportar CSV</button>
        </div>
      </section>

      <section className="analyst-demand-forecast-toolbar">
        <label className="analyst-demand-forecast-search">Buscar
      ...
      <section className="analyst-demand-forecast-summary">
      ...
      <section className="analyst-demand-forecast-table-card">
        <div className="analyst-demand-forecast-table-head">
          <div>
            <span>Vista operativa</span>
            <h3>Forecasting de demanda</h3>
          </div>
          <small>{filteredRows.length} resultados</small>
        </div>
        <div className="analyst-demand-forecast-table-wrap">
          <table>
            <thead>
              <tr>
                  ...
                  {columns.map((column) => (
                    <td key={column.key}>
                      {["estado", "prioridad", "riesgo", "rotacion", "severidad", "probabilidad", "concentracion", "escenario"].includes(column.key)
                        ? <span className={`analyst-demand-forecast-badge ${badgeClass(row[column.key])}`}>{row[column.key]}</span>
                        : row[column.key]}
                    </td>
                  ))}
                  <td>
                    <div className="analyst-demand-forecast-row-actions">
                      {actions.map((action) => <button key={action.id} onClick={() => runAction(action.id, row)}>{action.label}</button>)}
                    </div>
                  </td>
                </tr>
              ))}
              {filteredRows.length === 0 && <tr><td colSpan={columns.length + 1} className="analyst-demand-forecast-empty">No hay resultados para los filtros seleccionados.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>

      {toast && <div className="analyst-demand-forecast-toast">{toast}</div>}

      {modal?.type === "detail" && (
        <div className="analyst-demand-forecast-modal-backdrop" onClick={() => setModal(null)}>
          <section className="analyst-demand-forecast-modal" onClick={(event) => event.stopPropagation()}>
            <button className="analyst-demand-forecast-close" onClick={() => setModal(null)}>×</button>
            <span className="analyst-demand-forecast-eyebrow">Detalle</span>
            <h3>Información del registro</h3>
            <div className="analyst-demand-forecast-detail-grid">
              {columns.map((column) => (
            ...
            <div className="analyst-demand-forecast-modal-actions"><button onClick={() => setModal(null)}>Cerrar</button></div>
          </section>
        </div>
      )}
      ...
      {modal?.type === "form" && (
        <div className="analyst-demand-forecast-modal-backdrop" onClick={() => setModal(null)}>
          <form className="analyst-demand-forecast-modal analyst-demand-forecast-form" onSubmit={saveForm} onClick={(event) => event.stopPropagation()}>
            <button type="button" className="analyst-demand-forecast-close" onClick={() => setModal(null)}>×</button>
            <span className="analyst-demand-forecast-eyebrow">Formulario</span>
            <h3>{modal.title}</h3>
            <div className="analyst-demand-forecast-form-grid">
            ...
            <div className="analyst-demand-forecast-modal-actions">
