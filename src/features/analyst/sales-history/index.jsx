...
const formFields = ["producto", "region", "periodo", "unidades", "ticket", "margen", "crecimiento", "rotacion"];
...
export default function AnalystSalesHistoryPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      const matchesText = !term || normalize((row.producto ?? "") + " " + (row.region ?? "") + " " + (row.periodo ?? "") + " " + (row.unidades ?? "") + " " + (row.ticket ?? "") + " " + (row.margen ?? "") + " " + (row.crecimiento ?? "") + " " + (row.rotacion ?? "")).includes(term);
  ...
  function runAction(actionId, row) {
    ...
    if (actionId === "download") { downloadCsv("analyst-sales-history.csv", [row]); showToast("Archivo CSV descargado"); return; }
    showToast("Acción simulada correctamente");
  }

  return (
    <main className="analyst-sales-history-page">
      <section className="analyst-sales-history-hero">
        <div>
          <span className="analyst-sales-history-eyebrow">Analista comercial</span>
          <h2>Ventas históricas</h2>
          <p>Patrones de venta, margen y rotación. Este módulo tiene su propio código, estilos, estados, filtros, tabla y ventana dentro de <strong>src/features/analyst/sales-history</strong>.</p>
        </div>
        <div className="analyst-sales-history-hero-actions">

          <button onClick={() => downloadCsv("analyst-sales-history.csv", filteredRows)}>Exportar CSV</button>
        </div>
      </section>

      <section className="analyst-sales-history-toolbar">
        <label className="analyst-sales-history-search">Buscar
      ...
      <section className="analyst-sales-history-summary">
      ...
      <section className="analyst-sales-history-table-card">
        <div className="analyst-sales-history-table-head">
          <div>
        ...
        <div className="analyst-sales-history-table-wrap">
          <table>
            <thead>
              <tr>
                  ...
                  {columns.map((column) => (
                    <td key={column.key}>
                      {["estado", "prioridad", "riesgo", "rotacion", "severidad", "probabilidad", "concentracion", "escenario"].includes(column.key)
                        ? <span className={`analyst-sales-history-badge ${badgeClass(row[column.key])}`}>{row[column.key]}</span>
                        : row[column.key]}
                    </td>
                  ))}
                  <td>
                    <div className="analyst-sales-history-row-actions">
                      {actions.map((action) => <button key={action.id} onClick={() => runAction(action.id, row)}>{action.label}</button>)}
                    </div>
                  </td>
                </tr>
              ))}
              {filteredRows.length === 0 && <tr><td colSpan={columns.length + 1} className="analyst-sales-history-empty">No hay resultados para los filtros seleccionados.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>

      {toast && <div className="analyst-sales-history-toast">{toast}</div>}

      {modal?.type === "detail" && (
        <div className="analyst-sales-history-modal-backdrop" onClick={() => setModal(null)}>
          <section className="analyst-sales-history-modal" onClick={(event) => event.stopPropagation()}>
            <button className="analyst-sales-history-close" onClick={() => setModal(null)}>×</button>
            <span className="analyst-sales-history-eyebrow">Detalle</span>
            <h3>Información del registro</h3>
            <div className="analyst-sales-history-detail-grid">
              {columns.map((column) => (
            ...
            <div className="analyst-sales-history-modal-actions"><button onClick={() => setModal(null)}>Cerrar</button></div>
          </section>
        </div>
      )}
      ...
      {modal?.type === "form" && (
        <div className="analyst-sales-history-modal-backdrop" onClick={() => setModal(null)}>
          <form className="analyst-sales-history-modal analyst-sales-history-form" onSubmit={saveForm} onClick={(event) => event.stopPropagation()}>
            <button type="button" className="analyst-sales-history-close" onClick={() => setModal(null)}>×</button>
            <span className="analyst-sales-history-eyebrow">Formulario</span>
            <h3>{modal.title}</h3>
            <div className="analyst-sales-history-form-grid">
            ...
            <div className="analyst-sales-history-modal-actions">
