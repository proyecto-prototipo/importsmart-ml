...
const initialRows = [
  ...
  {
    "id": 3,
    "reporte": "Stock crítico y reposición",
    "tipo": "Operativo",
    "periodo": "Mayo 2026",
...
const actions = [{"id": "view", "label": "Ver detalle"}, {"id": "edit", "label": "Editar"}, {"id": "download", "label": "Descargar"}, {"id": "generate", "label": "Regenerar"}];
...
export default function AnalystReportsPage() {
  ...
  function runAction(actionId, row) {
    ...
    if (actionId === "download") { downloadCsv("analyst-reports.csv", [row]); showToast("Archivo CSV descargado"); return; }
    showToast("Acción simulada correctamente");
  }

  return (
    <main className="analyst-reports-page">
      <section className="analyst-reports-hero">
        <div>
          <span className="analyst-reports-eyebrow">Analista comercial</span>
          <h2>Reportes analíticos</h2>
          <p>Informes para revisión interna. Este módulo tiene su propio código, estilos, estados, filtros, tabla y ventana dentro de <strong>src/features/analyst/reports</strong>.</p>
        </div>
        <div className="analyst-reports-hero-actions">
          <button onClick={openCreate}>Generar reporte</button>
          <button onClick={() => downloadCsv("analyst-reports.csv", filteredRows)}>Exportar CSV</button>
        </div>
      </section>

      <section className="analyst-reports-toolbar">
        <label className="analyst-reports-search">Buscar
      ...
      <section className="analyst-reports-summary">
      ...
      <section className="analyst-reports-table-card">
        <div className="analyst-reports-table-head">
          <div>
        ...
        <div className="analyst-reports-table-wrap">
          <table>
            <thead>
              <tr>
                  ...
                  {columns.map((column) => (
                    <td key={column.key}>
                      {["estado", "prioridad", "riesgo", "rotacion", "severidad", "probabilidad", "concentracion", "escenario"].includes(column.key)
                        ? <span className={`analyst-reports-badge ${badgeClass(row[column.key])}`}>{row[column.key]}</span>
                        : row[column.key]}
                    </td>
                  ))}
                  <td>
                    <div className="analyst-reports-row-actions">
                      {actions.map((action) => <button key={action.id} onClick={() => runAction(action.id, row)}>{action.label}</button>)}
                    </div>
                  </td>
                </tr>
              ))}
              {filteredRows.length === 0 && <tr><td colSpan={columns.length + 1} className="analyst-reports-empty">No hay resultados para los filtros seleccionados.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>

      {toast && <div className="analyst-reports-toast">{toast}</div>}

      {modal?.type === "detail" && (
        <div className="analyst-reports-modal-backdrop" onClick={() => setModal(null)}>
          <section className="analyst-reports-modal" onClick={(event) => event.stopPropagation()}>
            <button className="analyst-reports-close" onClick={() => setModal(null)}>×</button>
            <span className="analyst-reports-eyebrow">Detalle</span>
            <h3>Información del registro</h3>
            <div className="analyst-reports-detail-grid">
              {columns.map((column) => (
            ...
            <div className="analyst-reports-modal-actions"><button onClick={() => setModal(null)}>Cerrar</button></div>
          </section>
        </div>
      )}
      ...
      {modal?.type === "form" && (
        <div className="analyst-reports-modal-backdrop" onClick={() => setModal(null)}>
          <form className="analyst-reports-modal analyst-reports-form" onSubmit={saveForm} onClick={(event) => event.stopPropagation()}>
            <button type="button" className="analyst-reports-close" onClick={() => setModal(null)}>×</button>
            <span className="analyst-reports-eyebrow">Formulario</span>
            <h3>{modal.title}</h3>
            <div className="analyst-reports-form-grid">
            ...
            <div className="analyst-reports-modal-actions">
