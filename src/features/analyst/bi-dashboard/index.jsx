...
export default function AnalystBiDashboardPage() {
  ...
  function runAction(actionId, row) {
    ...
    if (actionId === "download") { downloadCsv("analyst-bi-dashboard.csv", [row]); showToast("Archivo CSV descargado"); return; }
    showToast("Acción simulada correctamente");
  }

  return (
    <main className="analyst-bi-dashboard-page">
      <section className="analyst-bi-dashboard-hero">
        <div>
          <span className="analyst-bi-dashboard-eyebrow">Analista comercial</span>
          <h2>Dashboard BI</h2>
          <p>Gráficos, semáforos y filtros analíticos. Este módulo tiene su propio código, estilos, estados, filtros, tabla y ventana dentro de <strong>src/features/analyst/bi-dashboard</strong>.</p>
        </div>
        <div className="analyst-bi-dashboard-hero-actions">

          <button onClick={() => downloadCsv("analyst-bi-dashboard.csv", filteredRows)}>Exportar CSV</button>
        </div>
      </section>

      <section className="analyst-bi-dashboard-toolbar">
        <label className="analyst-bi-dashboard-search">Buscar
      ...
      <section className="analyst-bi-dashboard-summary">
      ...
      <section className="analyst-bi-dashboard-table-card">
        <div className="analyst-bi-dashboard-table-head">
          <div>
        ...
        <div className="analyst-bi-dashboard-table-wrap">
          <table>
            <thead>
              <tr>
                  ...
                  {columns.map((column) => (
                    <td key={column.key}>
                      {["estado", "prioridad", "riesgo", "rotacion", "severidad", "probabilidad", "concentracion", "escenario"].includes(column.key)
                        ? <span className={`analyst-bi-dashboard-badge ${badgeClass(row[column.key])}`}>{row[column.key]}</span>
                        : row[column.key]}
                    </td>
                  ))}
                  <td>
                    <div className="analyst-bi-dashboard-row-actions">
                      {actions.map((action) => <button key={action.id} onClick={() => runAction(action.id, row)}>{action.label}</button>)}
                    </div>
                  </td>
                </tr>
              ))}
              {filteredRows.length === 0 && <tr><td colSpan={columns.length + 1} className="analyst-bi-dashboard-empty">No hay resultados para los filtros seleccionados.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>

      {toast && <div className="analyst-bi-dashboard-toast">{toast}</div>}

      {modal?.type === "detail" && (
        <div className="analyst-bi-dashboard-modal-backdrop" onClick={() => setModal(null)}>
          <section className="analyst-bi-dashboard-modal" onClick={(event) => event.stopPropagation()}>
            <button className="analyst-bi-dashboard-close" onClick={() => setModal(null)}>×</button>
            <span className="analyst-bi-dashboard-eyebrow">Detalle</span>
            <h3>Información del registro</h3>
            <div className="analyst-bi-dashboard-detail-grid">
              {columns.map((column) => (
            ...
            <div className="analyst-bi-dashboard-modal-actions"><button onClick={() => setModal(null)}>Cerrar</button></div>
          </section>
        </div>
      )}
      ...
      {modal?.type === "form" && (
        <div className="analyst-bi-dashboard-modal-backdrop" onClick={() => setModal(null)}>
          <form className="analyst-bi-dashboard-modal analyst-bi-dashboard-form" onSubmit={saveForm} onClick={(event) => event.stopPropagation()}>
            <button type="button" className="analyst-bi-dashboard-close" onClick={() => setModal(null)}>×</button>
            <span className="analyst-bi-dashboard-eyebrow">Formulario</span>
            <h3>{modal.title}</h3>
            <div className="analyst-bi-dashboard-form-grid">
            ...
            <div className="analyst-bi-dashboard-modal-actions">
